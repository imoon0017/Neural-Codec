#!/usr/bin/env bash
# split_test_v2.sh — Split-test CurveCodecV2 across model configurations
#
# Identical split conditions to split_test.sh but uses the V2 architecture
# (EncoderAA + DecoderTC) via model.arch: curve_codec_v2 in each config.
#
# Split conditions
#   latent_dim        : 32  16  8
#   compaction_ratio  :  8  16
#   quantizer_bits    :  8
#   Total             : 6 combinations
#
# Usage:
#   ./scripts/split_test_v2.sh                                # default base config, cuda
#   ./scripts/split_test_v2.sh --base-config path/to/cfg.yaml
#   ./scripts/split_test_v2.sh --device cpu
#   ./scripts/split_test_v2.sh --dry-run                      # generate configs only
#   ./scripts/split_test_v2.sh --resume                       # resume each run from last.pt
#
# Outputs:
#   train/config/split_test_v2/v2_split_D{D}_c{c}_B{B}.yaml   generated configs
#   checkpoints/v2_split_D{D}_c{c}_B{B}/train.log              per-run log
#   checkpoints/v2_split_D{D}_c{c}_B{B}/best.pt                best checkpoint
#   checkpoints/split_test_v2_summary.json                     final results table
...
SPLIT_CONFIG_DIR="${PROJECT_ROOT}/train/config/split_test_v2"
CHECKPOINT_ROOT="${PROJECT_ROOT}/checkpoints"
SUMMARY_PATH="${CHECKPOINT_ROOT}/split_test_v2_summary.json"
RESULTS_FILE="${CHECKPOINT_ROOT}/.split_test_v2_results.jsonl"
...
for D, c, B in product(latent_dims, compaction_ratios, quantizer_bits):
    ...
    cfg["model"]["arch"]             = "curve_codec_v2"
    cfg["model"]["latent_dim"]       = D
    cfg["model"]["compaction_ratio"] = c
    cfg["model"]["quantizer_bits"]   = B
    cfg["paths"]["run_id"]           = f"v2_split_D{D}_c{c}_B{B}"

    out = split_cfg_dir / f"v2_split_D{D}_c{c}_B{B}.yaml"
...
for B in "${QUANTIZER_BITS[@]}"; do
    ...
    python - "${CKPT_DIR}/best.pt" "${RUN_ID}" "${D}" "${c}" "${B}" \
             ...
             "${RESULTS_FILE}" \
             "train/config/split_test_v2/${RUN_ID}.yaml" \
...
record = {
    "run_id": run_id, "arch": "curve_codec_v2",
    "D": D, "c": c, "B": B,
...
echo "============================================================"
echo "  CurveCodecV2 Split-test complete — $(date '+%Y-%m-%d %H:%M:%S')"
...
header = f"{'run_id':<33}  {'status':<8}  {'best_val_loss':>14}  {'duration':>10}"
sep    = "-" * len(header)
print(sep)
print(header)
print(sep)
for r in results:
    ...
    print(f"{r['run_id']:<33}  {r['status']:<8}  {bvl:>14}  {dur:>10}")
print(sep)
success = sum(1 for r in results if r["status"] == "success")
