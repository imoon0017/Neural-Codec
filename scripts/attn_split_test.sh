#!/usr/bin/env bash
# attn_split_test.sh — Split-test CurveCodecAttn across latent_dim, compaction_ratio, quantizer_bits
#
# Fixed:    arch=curve_codec_attn, attn_heads=4, attn_window_size=7,
#           encoder_channels=64, batch_size=1, epochs=200
# Variable: latent_dim        (8, 16, 32)
#           compaction_ratio  (8, 16, 32)
#           quantizer_bits    (8, 16)
#           Total: 18 combinations
#
# NOTE: patch_size_px must be divisible by compaction_ratio.  With the default
#       dataset (S=840), c=16 and c=32 require a dataset re-rasterized to a
#       compatible patch size (e.g. S=896 for c=32, or S=832 for c=16).
#       Use --dry-run to generate configs before committing to a full training run.
#
# Usage:
#   ./scripts/attn_split_test.sh                                   # cuda
#   ./scripts/attn_split_test.sh --base-config path/to/cfg.yaml
#   ./scripts/attn_split_test.sh --device cpu
#   ./scripts/attn_split_test.sh --dry-run                         # generate configs only
#   ./scripts/attn_split_test.sh --resume                          # resume from last.pt
#
# Outputs:
#   train/config/attn_split/attn_D{D}_c{c}_B{B}.yaml    generated configs
#   checkpoints/attn_D{D}_c{c}_B{B}/train.log            per-run log
#   checkpoints/attn_D{D}_c{c}_B{B}/best.pt              best checkpoint
#   checkpoints/attn_split_summary.json                   final results table
...
SUMMARY_PATH="${CHECKPOINT_ROOT}/attn_split_summary.json"
RESULTS_FILE="${CHECKPOINT_ROOT}/.attn_split_results.jsonl"
...
echo "============================================================"
echo "  CurveCodecAttn Split Test"
echo "  Base config       : ${BASE_CONFIG}"
echo "  Device            : ${DEVICE}"
echo "  Dry run           : $([ ${DRY_RUN} -eq 1 ] && echo yes || echo no)"
echo "  Resume            : $([ ${RESUME} -eq 1 ] && echo yes || echo no)"
echo "  attn_heads        : ${ATTN_HEADS}  (fixed)"
echo "  attn_window_size  : ${ATTN_WINDOW_SIZE}  (fixed)"
echo "  encoder_channels  : ${ENCODER_CHANNELS}  (fixed)"
...
attn_heads        = int(sys.argv[4])
attn_window_size  = int(sys.argv[5])
encoder_channels  = int(sys.argv[6])
batch_size        = int(sys.argv[7])
latent_dims       = [int(x) for x in sys.argv[8].split()]
compaction_ratios = [int(x) for x in sys.argv[9].split()]
quantizer_bits_l  = [int(x) for x in sys.argv[10].split()]

with open(base_config) as f:
    base_cfg = yaml.safe_load(f)

count = 0
for D, c, B in product(latent_dims, compaction_ratios, quantizer_bits_l):
    # attn_heads must divide D
    if D % attn_heads != 0:
        print(f"  SKIP  attn_D{D}_c{c}_B{B} — D={D} not divisible by attn_heads={attn_heads}")
        continue
    ...
    cfg["model"]["attn_heads"]       = attn_heads
    cfg["model"]["attn_window_size"] = attn_window_size
    cfg["model"]["encoder_channels"] = encoder_channels
    cfg["training"]["batch_size"]    = batch_size
    run_id = f"attn_D{D}_c{c}_B{B}"
    cfg["paths"]["run_id"] = run_id

    out = split_cfg_dir / f"{run_id}.yaml"
...
for B in "${QUANTIZER_BITS_LIST[@]}"; do
    ...
    if [[ ! -f "${CFG_PATH}" ]]; then
        echo "  SKIP — config not found (likely D not divisible by attn_heads): ${CFG_PATH}"
        echo ""
        continue
    fi
    ...
    python - "${CKPT_DIR}/best.pt" "${RUN_ID}" "${D}" "${c}" "${B}" \
             ...
             "${RESULTS_FILE}" \
             "train/config/attn_split/${RUN_ID}.yaml" \
...
echo "============================================================"
echo "  Attn split-test complete — $(date '+%Y-%m-%d %H:%M:%S')"
