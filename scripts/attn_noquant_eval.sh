#!/usr/bin/env bash
# attn_noquant_eval.sh — Evaluate the CurveCodecAttn checkpoint (no quantization)
#
# Model : CurveCodecAttn — conv encoder + windowed self-attention bottleneck
# Config: train/config/attn_noquant_v1.yaml
#         D=32, c=8, heads=4, window=7, batch=1, quantize=false
#
# The quantizer is bypassed: encoder→decoder runs in memory without the
# .cdna roundtrip.  Compression ratio is not reported (no compressed
# artifact is produced).  Use this result as the quality ceiling for the
# attention architecture.
#
# Results written to:
#   eval/results/attn_noquant_v1/       best checkpoint
#   eval/results/attn_noquant_v1_last/  last checkpoint (optional, --last flag)
#
# Usage:
#   ./scripts/attn_noquant_eval.sh                        # default: cuda, validation split
#   ./scripts/attn_noquant_eval.sh --device cpu
#   ./scripts/attn_noquant_eval.sh --split test
#   ./scripts/attn_noquant_eval.sh --last                 # also eval last.pt
#   ./scripts/attn_noquant_eval.sh --inspection           # write per-marker .oas files
#   ./scripts/attn_noquant_eval.sh --dry-run
...
echo "============================================================"
echo "  CurveCodecAttn Evaluation  (no quantization)"
echo "  Run ID     : ${RUN_ID}"
echo "  Config     : ${CONFIG}"
...
echo "  Note       : CR not reported — quantizer bypassed, no .cdna written"
...
def print_summary(label, run_dir):
    ...
    print(f"    CR  mean  : N/A (quantizer bypassed)")
